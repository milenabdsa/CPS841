...
for dataset_type in ["train", "val"]:  # Change to "val" for validation data
    ...
    for start, end in ranges:
        ...
        filename_X = f"{filename_prefix}X.p"
        filename_y = f"{filename_prefix}y.p"
        
        # Check if files exist before loading
        if not os.path.exists(filename_X) or not os.path.exists(filename_y):
            print(f"Warning: {filename_prefix} files not found, skipping...")
            continue
        
        print(f"Loading {filename_prefix}...")
        X_chunk = pickle.load(open(filename_X, "rb"))
        y_chunk = pickle.load(open(filename_y, "rb"))
